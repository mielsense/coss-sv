...
let { children, class: className, ref = $bindable(null), ...props }: SidebarContentProps = $props();
</script>
<ScrollArea class="min-h-0 flex-1" fill overscrollContain scrollFade>
  <div
    bind:this={ref}
    class={cn("flex h-full flex-col gap-2 group-data-[collapsible=icon]:overflow-hidden", className)}
