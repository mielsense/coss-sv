...
export type TableBodyProps = Omit<SvelteHTMLElements["tbody"], "children" | "class"> & {
  children?: Snippet;
  class?: string;
...
let { children, class: className, ref = $bindable(null), ...props }: TableBodyProps = $props();
</script>

<tbody bind:this={ref} class={cn(baseClass, className)} data-slot="table-body" {...props}>
  {@render children?.()}
</tbody>
