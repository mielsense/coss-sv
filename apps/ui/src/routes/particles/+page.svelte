<script lang="ts">
  ...
  import ParticleCard from "$lib/particles/ParticleCard.svelte";
  import ParticleSearchField from "$lib/particles/ParticleSearchField.svelte";
  import { particleCatalog } from "$lib/particles/catalog.js";
  import {
    filterParticles,
    getParticleSearchItems,
    parseParticleTags,
  } from "$lib/particles/model.js";
  import { particlePreviewRegistry } from "../preview/[name]/preview-registry.js";
  import type { Component } from "svelte";

  const availableParticles = particleCatalog.filter(({ name }) => name in particlePreviewRegistry);
  ...
  function particleComponent(name: string): Component {
    const entry = particlePreviewRegistry[name];
  ...
  {:else if parsedTags.valid.length > 0}
    ...
    >
      {#each filteredParticles as particle (particle.name)}
        <ParticleCard component={particleComponent(particle.name)} {particle} />
      {/each}
    </div>
  {/if}
</div>
