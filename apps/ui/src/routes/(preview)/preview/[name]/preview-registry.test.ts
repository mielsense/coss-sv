...
describe("typed particle preview registry", () => {
  test("discovers particle modules without loading them until the requested preview resolves", async () => {
    let loadCount = 0;
    const registry = createParticlePreviewLoaderRegistry({
      "../../../../registry/default/particles/p-button-1.svelte": async () => {
        loadCount += 1;
        return {
    ...
    expect(registry["p-button-1"]?.modulePath).toBe(
      "../../../../registry/default/particles/p-button-1.svelte",
    );

    await expect(loadParticlePreview(registry, "p-button-1")).resolves.toEqual({
  ...
  test.each([
  ...
  ])("rejects %s", async (_label, meta, message) => {
    const registry = createParticlePreviewLoaderRegistry({
      "../../../../registry/default/particles/p-button-1.svelte": async () => ({
        default: ComponentFixture,
        meta,
    ...
    await expect(loadParticlePreview(registry, "p-button-1")).rejects.toThrow(message);
  });
  ...
  test("discovers every authored file from the locked 508-particle ownership inventory", () => {
    ...
    const report = compareParticleInventory(authoredIds, particlePreviewLoaders);

    expect(report).toEqual({ complete: true, extra: [], missing: [] });
    for (const [id, entry] of Object.entries(particlePreviewLoaders)) {
      expect(entry.modulePath.endsWith(`/${id}.svelte`)).toBe(true);
