      ...
      ><Drawer.Panel
        ><Drawer.Menu>
              ...
              ><Drawer.Panel
                ><Drawer.Menu
                  ><Drawer.MenuGroup
                    ...
                    ></Drawer.MenuGroup
                  ><Drawer.Close class={drawerMenuItemVariants()}>Jazz</Drawer.Close
                  ><Drawer.Root
                    ...
                    ><Drawer.Popup showBar
                      ><Drawer.Panel
                        ><Drawer.Menu
                          ><Drawer.MenuGroup
                            ><Drawer.MenuGroupLabel>Rock</Drawer.MenuGroupLabel></Drawer.MenuGroup
                          ><Drawer.Close class={drawerMenuItemVariants()}>Hard Rock</Drawer.Close
                          ><Drawer.Close class={drawerMenuItemVariants()}>Soft Rock</Drawer.Close
                          ><Drawer.Close class={drawerMenuItemVariants()}
                            >Classic Rock</Drawer.Close
                          ><Drawer.MenuSeparator />
                          {#each ["Metal", "Punk", "Grunge", "Alternative", "Indie", "Electronic"] as item}
                            <Drawer.Close class={drawerMenuItemVariants()}>{item}</Drawer.Close>
                          {/each}</Drawer.Menu
                        ></Drawer.Panel
                      ></Drawer.Popup
                    ></Drawer.Root
                  ><Drawer.Close class={drawerMenuItemVariants()}>Pop</Drawer.Close></Drawer.Menu
                ></Drawer.Panel
              ></Drawer.Popup
            ></Drawer.Root
          >
          ...
          <Drawer.MenuGroup
            ...
            ><Drawer.Close class={drawerMenuItemVariants({ variant: "destructive" })}
              ><HugeiconsIcon
                aria-hidden="true"
                icon={Delete02Icon}
                strokeWidth={2}
...
<div class="hidden md:block">
  <Menu.Root>
    <Menu.Trigger
      aria-label="Open menu"
      class={buttonVariants({ size: "icon", variant: "outline" })}
      ><HugeiconsIcon aria-hidden="true" icon={MoreHorizontalIcon} strokeWidth={2} /></Menu.Trigger
    >
    <Menu.Popup>
      <Menu.Group
        ><Menu.GroupLabel>Actions</Menu.GroupLabel
        ><Menu.Item
          ><HugeiconsIcon aria-hidden="true" icon={PencilIcon} strokeWidth={2} />Edit</Menu.Item
        ><Menu.Item
          ><HugeiconsIcon aria-hidden="true" icon={CopyIcon} strokeWidth={2} />Duplicate</Menu.Item
        ><Menu.Item
          ><HugeiconsIcon aria-hidden="true" icon={Share08Icon} strokeWidth={2} />Share</Menu.Item
        ></Menu.Group
      >
      ...
      <Menu.Group
        ...
        ><Menu.Item variant="destructive"
          ><HugeiconsIcon aria-hidden="true" icon={Delete02Icon} strokeWidth={2} />Delete</Menu.Item
        ></Menu.Group
      >
    </Menu.Popup>
  </Menu.Root>
