...
<Dialog.Root open={dialogOpen} onOpenChange={changeDialog}>
  ...
  <Dialog.Popup showCloseButton={false}>
    ...
    >
    <Form class="contents" onsubmit={(event) => { event.preventDefault(); dialogOpen = false; }}>
      <Dialog.Panel><Field.Root><Textarea bind:value /></Field.Root></Dialog.Panel>
      <Dialog.Footer>
        <Dialog.Close class={buttonVariants({ variant: "ghost" })}>Cancel</Dialog.Close>
        <Button onclick={() => { value = ""; dialogOpen = false; }}>Send</Button>
      </Dialog.Footer>
    </Form>
  </Dialog.Popup>
  <AlertDialog.Root open={confirmOpen} onOpenChange={(open) => (confirmOpen = open)}>
    <AlertDialog.Popup>
      ...
      >
      <AlertDialog.Footer
        ><AlertDialog.Close class={buttonVariants({ variant: "ghost" })}>Go back</AlertDialog.Close
        ><Button onclick={discard}>Discard</Button></AlertDialog.Footer
      >
    </AlertDialog.Popup>
  </AlertDialog.Root>
</Dialog.Root>
