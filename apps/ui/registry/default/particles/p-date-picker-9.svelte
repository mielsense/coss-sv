<script module lang="ts">
  ...
  export const meta = defineParticleMeta({
    ...
    interactive: true,
    responsive: true,
    title: "Two-month date range picker",
  });
</script>
...
<Popover.Root>
  <Popover.Trigger class={buttonVariants({ class: "w-full justify-start", variant: "outline" })}>
    ...
    />{#if date?.from}{format(date.from)}{#if date.to}
        - {format(date.to)}{/if}{:else}<span>Pick a date range</span>{/if}
  </Popover.Trigger><Popover.Popup>
    <Calendar defaultMonth={date?.from} mode="range" numberOfMonths={2} bind:selected={date} />
  </Popover.Popup>
</Popover.Root>
