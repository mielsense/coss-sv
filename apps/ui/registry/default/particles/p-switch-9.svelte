(eval):5: parse error near `end' (eval):5: parse error near `end'
...
{/snippet}
{#snippet timeCombobox(ariaLabel: string, id: string, items: string[], value: string, onChange: (time: string) => void)}
  <Combobox.Root
    autoHighlight
    {items}
