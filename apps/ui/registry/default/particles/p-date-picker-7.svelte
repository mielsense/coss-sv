...
<Popover.Root>
  <Popover.Trigger class={buttonVariants({ class: "w-full justify-start", variant: "outline" })}>
    <HugeiconsIcon class="shrink-0" icon={Calendar03Icon} aria-hidden="true" strokeWidth={2} />
    <span class="flex flex-wrap gap-1">
      {#if dates?.length}{#each dates.slice(0, 3) as selected (selected.toISOString())}<Badge
            variant="secondary"
          >
            {format(selected)}
          </Badge>{/each}{#if dates.length > 3}<Badge variant="secondary">
            +{dates.length - 3}
          </Badge>{/if}{:else}<span>Pick dates</span>{/if}
    </span>
  </Popover.Trigger><Popover.Popup>
    <Calendar mode="multiple" bind:selected={dates} />
  </Popover.Popup>
</Popover.Root>
