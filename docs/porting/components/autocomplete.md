...
The D8 repair restores the upstream `will_error` branch in `p-autocomplete-12`: the fixed real timer rejects deterministically, stale requests are ignored, loading clears, results reset, and the status displays the exact `Failed to fetch movies. Please try again.` copy. Unit, browser, reduced-motion, and built production-route coverage exercise the error state with real timers.
