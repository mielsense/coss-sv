...
Fresh inspection covered the complete Textarea page and `p-textarea-1` through `p-textarea-15`. The page preserves the six upstream previews in order, while particles 7 through 15 remain registry examples rather than being inserted into the page. Usage, Field composition, API copy, native attributes, sizes, validation, and displayed source were translated to the public Svelte namespace without changing visible copy.

All 15 particles are part of the exact D6 compiler, SSR, metadata, icon-authority, and responsive light/dark browser inventory.
