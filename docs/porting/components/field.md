...
The focused browser regression exposes a coordinator-owned package defect rather than hiding it in a particle: in `p-field-1`, `Field.Label` renders `for="c3"` while the Input renders a different generated ID, and the Input does not receive the Description ID through `aria-describedby`. The red browser assertions require the label and description relationships to reach the native control. D6 leaves the exact upstream particle source intact pending the reviewed Field package repair.
