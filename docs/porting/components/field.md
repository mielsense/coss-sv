...
`Field.Root as="fieldset"` now writes `aria-labelledby` during SSR. Its generated `legendId` is also the default ID for `Fieldset.Legend`, so the relationship is correct before hydration. Consumers who give the Legend an explicit ID pass the same value as `legendId` on the Root. SSR tests cover both generated and explicit IDs without adding a wrapper element.
