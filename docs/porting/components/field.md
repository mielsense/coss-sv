...
`apps/ui/src/lib/parity/components/field.svelte` reproduces all nine dependency-free Field particles in source order: `p-field-1` through `p-field-6`, `p-field-10`, `p-field-12`, and `p-field-15`. Each particle has an exact `id` and `data-particle` review selector. The wrapper preserves the registry preview metadata: `p-field-1` through `p-field-6`, `p-field-10`, and `p-field-12` use the upstream full-width, `max-w-64` preview constraint; `p-field-15` has no registry width override. The validity particle keeps the live error and JSON state view, and the remaining fixtures preserve the source labels, copy, child order, disabled state, and integrated controls.

Nine Field particles remain dependency-gated after fresh import inspection: `p-field-7` requires Autocomplete; `p-field-8` and `p-field-9` require Combobox; `p-field-11` requires Select; `p-field-13` requires CheckboxGroup; `p-field-14` requires RadioGroup; `p-field-16` requires Slider; `p-field-17` requires NumberField; and `p-field-18` requires Select. Those unfinished wrappers are not replaced inside the fixture.
