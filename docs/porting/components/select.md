...
The icon-authority repair reread the COSS wrapper, the icon-bearing particles, and the matching Shards trigger, indicator, and scroll-arrow parts. Trigger, check, scroll, cable, category, and development icons now use `@hugeicons/svelte` with matching free-core assets; the parity fixture no longer substitutes text glyphs. Source regression coverage rejects inline SVG and Lucide markup. The separator's explicit Shards-compatible public contract restores its generated declaration and `ref` binding, so package-output coverage now verifies all three delegated Select parts.
