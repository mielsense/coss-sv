...
All 28 D6-owned Input Group particles are now ported: `p-input-group-1` through `p-input-group-24` and `p-input-group-26` through `p-input-group-29`. The page keeps the exact 17-example upstream order, including Number Field before Textarea. The remaining registry examples preserve their exact copy, icons, tooltips, menus, Select composition, loading state, password behavior, counters, and alignment.

The `p-input-group-17` repair re-read the complete COSS particle, Input Group page, registry record, and generated registry item. It also re-read the pinned Shards Toolbar, Menu, and Tooltip source, docs, demos, exported types, and focused tests. COSS composes Tooltip Trigger, Menu Trigger, and Button props onto one button. The Svelte port now does the same through the public `Tooltip.createTriggerAttachment` API: `Menu.Trigger` owns the button and menu behavior, `buttonVariants` supplies the exact Button classes, and the attachment adds tooltip focus, hover, description, and trigger state without wrapper DOM. The separate Send control remains a real `Button` with the same attachment. Both controls use hydration-stable IDs and official Hugeicons data through the central renderer. There is no remaining `p-input-group-17` composition deviation.

The focused browser regression checks the two-button DOM, tooltip description, menu ARIA state, exact menu items, arrow-key focus, Escape dismissal, and focus restoration. The attachment also replaces the earlier wrapper before the page reaches production preview output.
