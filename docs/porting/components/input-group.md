...
`p-input-group-17` needs one semantic-neutral `span` because the local Shards Tooltip Trigger owns its element and cannot merge trigger behavior onto the Menu Trigger. The wrapper is `inline-flex`, adds no visible geometry, and preserves the nested menu button’s accessible name and interaction. A `display: contents` trigger would remove a dependable hover/focus target, while nesting interactive trigger elements would be invalid. This is a DOM-only Shards composition deviation; visible and behavioral parity remain required.
