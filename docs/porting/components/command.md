...
The documentation lane freshly reread the complete permitted COSS Command MDX page, both Command particles, every Command consumer, and the complete Shards Autocomplete, Dialog, and Scroll Area materials that implement the Svelte primitive. The public page preserves the source's single `p-command-1` preview and exact palette copy. Both production modules are present: the dialog palette preserves its two groups, shortcut, focus, filtering, and close behavior; the AI example preserves its complete search and AI modes, loading delay, response, and three reference links without unsafe HTML injection.
