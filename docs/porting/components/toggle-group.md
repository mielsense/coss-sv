...
The D5 lane re-read the complete Toggle Group MDX page, particles `p-toggle-group-1` through `p-toggle-group-9`, the target namespace, and the complete matching local Shards source, tests, docs, and examples. The page keeps all nine preview IDs, their order, and the upstream Changelog section. The live page exposed 9 previews at 1440px; at 390px its h1 was x=16, y=88, 358×36 with no horizontal overflow. Particle 9 preserves the three accessible toggle labels and initial Bold selection.

Shards Tooltip Trigger cannot delegate its behavior to a Svelte component. Each item is therefore wrapped by a real Shards Trigger with `display: contents` and its own typed `Tooltip.Handle`. The wrapper creates no layout box and is not focusable, so the Toggle Group Item remains the only tab stop and Shards retains roving focus and pressed state. The popup anchors to the item's element. Focus waits for Shards' trigger registration flush; hover delay, Escape dismissal, click handling, and popup state remain inside Shards. Component-browser and production-preview tests cover the trigger registration, initial delay, focus, dismissal, pressed state, and focus order. Shared page metadata remains coordinator-owned.

## Central Hugeicons renderer migration

The p-toggle-group-1 through p-toggle-group-9 registry sources keep their audited Hugeicons core glyph data, two-pixel strokes, classes, and ARIA attributes. They now render that data with the public SSR-safe HugeiconsIcon exported by @coss-sv/ui. The focused ownership test enumerates each migrated particle, rejects the framework-specific renderer, checks every icon invocation, and verifies server-rendered SVG geometry.
