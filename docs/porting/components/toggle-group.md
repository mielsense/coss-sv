...
The D5 lane re-read the complete Toggle Group MDX page, particles `p-toggle-group-1` through `p-toggle-group-9`, the target namespace, and the complete matching local Shards source, tests, docs, and examples. The page keeps all nine preview IDs, their order, and the upstream Changelog section. The live page exposed 9 previews at 1440px; at 390px its h1 was x=16, y=88, 358×36 with no horizontal overflow. Particle 9 preserves the three accessible toggle labels and initial Bold selection. Shards Tooltip Trigger cannot delegate to a Svelte component, so each local Tooltip Root is controlled by the actual Toggle Group Item and its Popup is anchored to that item's ref. This avoids the extra nonfocusable trigger wrapper while retaining focus, hover, pressed, and roving-focus behavior. Shared page metadata remains coordinator-owned.
