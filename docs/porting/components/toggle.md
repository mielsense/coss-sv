...
Shards Tooltip Trigger cannot delegate its behavior to a Svelte component. The narrow local adaptation therefore renders the upstream `div` around the Toggle as the real Shards Trigger and controls it through a typed `Tooltip.Handle`. The trigger is `inline-flex`, so both the tooltip and anchored toast use the visible 32px toggle geometry rather than the preview's full width. Focus waits for the next Svelte flush before opening, which lets Shards register the detached trigger and cancels the open if focus has already moved. Shards still owns hover delay, Escape dismissal, click handling, popup state, and ARIA linkage. The wrapper is not focusable and the Toggle remains the only tab stop. Component-browser and production-preview tests cover those contracts. Shared page metadata remains a coordinator seam.
